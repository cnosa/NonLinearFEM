...
function SolNum1Picard(M,maxiter,ψ₀,A,δ,γ,tol)
...
#	f = [exp(γ* ((h[i+1]-h[i])*t/4 + h[i]) )+(h[i+1]-h[i])*t/4 + h[i] for t in 1:3]
		...
		hζ= ϕ1*hant[dof[1]]+ϕ2*hant[dof[2]]
		kζ= ϕ1*kant[dof[1]]+ϕ2*kant[dof[2]]
...
#		E[dof,dof]  = E[dof,dof] + Eloc
	...
	k= D\c
	h = D\b
		if (norm(k-kant)<tol && norm(h-hant)<tol)
			break
		end
...
begin
	...
	hsol,ksol,convergence=SolNum1Picard(M,maxiter,ψ₀,A,δ,γ, 0.01)
	
	
end

# ╔═╡ 273f0f88-a7b6-4667-8249-abcb9a6cdef3
convergence

# ╔═╡ a7fd9bcc-33d0-4f3b-8022-6644eb1127a2
	plot(plot(nodesp,hsol,label="ϕ", lw=4, ylims=(minimum(hsol)-.1, maximum(hsol)+.1)),
		plot(nodesp,ksol,label="ψ", lw=4,ylims=(minimum(ksol)-.1, maximum(ksol)+.1)),
...
manifest_format = "2.0"
project_hash = "cba49939d24119cd4a182ffd78af27d1e450bf99"
