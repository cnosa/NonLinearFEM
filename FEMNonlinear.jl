...
function SolNum1Picard(M,maxiter)
	...
	for i=1:M #Iteración por cada elemento
		...
		x1=2*π*i/M
		Δ=x1-x0
		f = [exp(γ* ((h[i+1]-h[i])*t/4 + h[i]) )+h[i] for t in 1:3]
		#Pesos y puntos de cuadratura locales
		ω = 0.5*(x1-x0)*ω_r
		ζ = [x0,x0,x0]+ 0.5*([1,1,1]+ζ_r)*(Δ) 
...
plot(plot(0:2*π/10:2*π,SolNum1Picard(10,1)[1]),
	plot(0:2*π/10:2*π,SolNum1Picard(10,1)[2]),
	layout=(1,2))
...
manifest_format = "2.0"
project_hash = "db08336e54c433ed47a81444095619f3020f00ee"
